...
class Icon extends _react.Component {
  ...
  getIcon(_data) {
    ...
    if (_data.type === 'Icons') {
      icon = /*#__PURE__*/_jsx(_UI.Icon, { icon: `${_data._id}`, size: `${getFaSize(size)}` });
    }

    if (_data.type === 'Flags') {
      const flagClassName = `flag-icon flag-icon-${_UI.CountryList.get(_data._id).cca2}`.toLowerCase();
