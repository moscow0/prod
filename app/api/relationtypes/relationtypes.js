...
_model.default.get().then((response) => {
...
function _update(newTemplate) {
  return _model.default.getById({ _id: newTemplate._id }).then((currentTemplate) => {
...

{
  ...
  async save(relationtype) {
    relationtype.properties = await (0, _utils.generateNames)(relationtype.properties || []);
  ...
  delete(id) {
    return _relationships.default.countByRelationType(id).then((relationshipsUsingIt) => {
