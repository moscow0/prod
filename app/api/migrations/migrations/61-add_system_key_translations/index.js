...
var _csv = _interopRequireDefault(require("../../../csv/csv"));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /*
...
async function readCsvToSystemKeys(db, filename) {const fstream = _fs.default.createReadStream(filename);const rows = await (0, _csv.default)(fstream).read();fstream.close();const translations = await db.collection('translations').find().toArray();const locales = translations.map((tr) => tr.locale);
